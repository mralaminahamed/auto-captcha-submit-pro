...
import {
    ...
    assembleAppContent,
    getLoggedInUsername,
    getLoggedInUsersEarned,
    getLoggedInUsersReferralsAttracted,
...
webMessageReceiver('mishusoft-payment-confirm', function (payload: { licence: any; }) {
    ...
    app.domain.extension.forEach(function (ext) {
        ...
        if (coll.dom.extAbility) {
            ...
            if (coll.dom.commonText === app.domain.commonText) {
                ...
                /*#!endif*/
                let loaderWindow: any = createElement([{
                    'div': {
                        'id': 'app-loader',
                        'class': 'app-window'
                    }
                ...
                if (coll.dom.currentPage === app.page.showadv) {
                    assembleAppContent();
                    (app.document.body as HTMLBodyElement).setAttribute('style', 'margin-top: 10%;');
                }
                ...
                if (coll.dom.currentPage === undefined || coll.dom.currentPage === app.page.home || coll.dom.currentPage === app.page.support || coll.dom.currentPage === app.page.fastpay) {
                    (app.document.body as HTMLBodyElement).setAttribute('style', 'margin-top: 0;');
                }
                ...
                if (coll.dom.currentPage === app.page.user_area) {
                    (app.document.body as HTMLBodyElement).setAttribute('style', 'margin-top: 3%;');
                }
                if (coll.dom.currentPage === app.page.help) {
                    (app.document.body as HTMLBodyElement).setAttribute('style', 'margin-top: 12%;');
                }
                if (coll.dom.currentPage === app.page.login || coll.dom.currentPage === app.page.restorepass || coll.dom.currentPage === app.page.registration || coll.dom.currentPage === app.page.checkpay || coll.dom.currentPage === app.page.updmoney || coll.dom.currentPage === app.page.payout || coll.dom.currentPage === app.page.aboutus) {
                    (app.document.body as HTMLBodyElement).setAttribute('style', 'margin-top: 15%;');
                }
...
function contentResponder(request: { constructor?: any; command?: any; data?: any; clientVerification?: any; message?: any; registration?: any; way?: any; login?: any; log_status?: any; licence?: any; passwordRecovery?: any; password?: any; clientIpData?: any; userdata?: any; }):any {
...
*/
            ...
            if (request.command === 'waitForNextDay') {
                if (captureElementById('app-welcome-window') === null || captureElementById('app-welcome-window') === undefined && captureElementById('app-waitForNextDay-window') === undefined) {
                    ...
                    let display: any = document.querySelector('#countdown-time');
                    startTimer(retrieveTimeByDate(request.data.nextUpdate) / 1000, (display as HTMLElement));
                }
            }
            ...
            if (request.registration !== undefined) {
                ...
                if (request.message === 'error') {
                    ...
                    if (request.registration === 'already_register') {
                        ...
                        if (request.way !== undefined && request.way === 'ip') {
                            /*console.log(request.way);*/
                            let u: any = createElement([{'u': {'style': 'color: white;'}}]);
                            let ip_message: any = createElement([{'strong': {}}]);
                ...
                if (request.message === 'success' && request.registration === 'new_register') {
                    ...
                    let infoTable: any = createElement([{'table': {'style': 'width:100%;'}}]);
                    let infoTableTbody: any = createElement([{'tbody': {}}]);
                    infoTable.appendChild(infoTableTbody);

                    let infoTableTbodyTr1: any = createElement([{'tr': {}}]);
                    infoTableTbody.appendChild(infoTableTbodyTr1);
                    let infoTableTbodyTr1Td1: any = createElement([{'td': {'style': 'width:95px;'}}]);
                    infoTableTbodyTr1Td1.textContent = 'Earning Limit:';
                    infoTableTbodyTr1.appendChild(infoTableTbodyTr1Td1);
                    let infoTableTbodyTr1Td2: any = createElement([{'td': {'id': 'earn-limit'}}]);
                    infoTableTbodyTr1.appendChild(infoTableTbodyTr1Td2);

                    let infoTableTbodyTr2: any = createElement([{'tr': {}}]);
                    infoTableTbody.appendChild(infoTableTbodyTr2);
                    let infoTableTbodyTr2Td1: any = createElement([{'td': {'style': 'width:95px;'}}]);
                    infoTableTbodyTr2Td1.textContent = 'Earned:';
                    infoTableTbodyTr2.appendChild(infoTableTbodyTr2Td1);
                    let infoTableTbodyTr2Td2: any = createElement([{'td': {'id': 'earn'}}]);
            ...
            if (request.login !== undefined) {
                ...
                if (request.message === 'success' && request.login === 'passed' && request.log_status === 'success') {
                    ...
                    if (request.licence.length === 0) {
                        let errorPanel: any = createElement([{
                            'div': {
                                'id': 'errorPanel',
                        ...
                        captureElementById('infoPanel').parentElement?.insertBefore(errorPanel, captureElementById('infoPanel'));
                        captureElementById('infoPanel').classList.add('ev_info');
                        ...
                        (document.getElementsByClassName('acsAppTitleText')[1] as HTMLElement).textContent = 'Licence Expired';
                        captureElementById('email-address').style.display = 'none';
                        captureElementById('acs-password').style.display = 'none';
...
function executeApplication(): any {
    app.body.validNum = '';
    if (captureElementById('captcha-img-zone') !== null || true) {
        captureElementById('captcha-img-zone')?.childNodes.forEach(function (element: HTMLElement) {
            if (element.nodeName === 'TD') {
                if ((element as HTMLElement).firstElementChild?.firstElementChild !== null || (element as HTMLElement).firstElementChild?.firstElementChild !== undefined && (element as HTMLElement).firstElementChild?.firstElementChild?.nodeName === 'IMG') {
                    let img_src_txt = ((element as HTMLElement).firstElementChild?.firstElementChild as HTMLImageElement).src;
...
captureElementById("auto-data-entry-start-btn").addEventListener("click", function () {
    ...
    if (this.textContent === 'Start') {
        ...
        if (captureElementById("data-entry-options").value === 'auto') {
            ...
            captureElementById("data-entry-options").setAttribute("disabled", "disabled");
            app.body.dataEntry.count = <any>setInterval(executeApplication, 1000);
        }
