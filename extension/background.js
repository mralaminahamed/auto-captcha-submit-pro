...
function setDefaultQAppData(ip, city, country_name) {
    browser.storage.local.set(createDefaultAppData(BrJS.BrowserNameFull, BrJS.BrowserVersion, ip, city, country_name, BrJS.DeviceName, BrJS.PlatformName, BrJS.PlatformArchitecture))
        /*#!if debug===true*/
        .then(() => {
...
export function checkSettings(details, ipdata) {
    checkAppInstallation(function (setting) {
        ...
        optimizeAppSettingObject(setting, [{ 'client': ['ip', 'city', 'country'] }], function (setting) {
            client.ip = setting.client.ip;
            client.city = setting.client.city;
            client.country = setting.client.country;
        }, installDefaultQAppData);
        optimizeAppSettingObject(setting, [{ 'app': ['id'] }], function (setting) {
            if (setting.app.id !== '') {
                optimizeAppSettingObject(setting, [{ 'app': ['name', 'version'] }], function (setting) {
        ...
        optimizeAppSettingObject(setting, [{ 'user': ['first_name', 'last_name', 'email', 'password'] }], function (setting) {
        ...
        optimizeAppSettingObject(setting, [{ 'licence': ['key', 'type', 'issue', 'update', 'nextUpdate', 'expire', 'limit', 'limitBase'] }], function (setting) {
    ...
    if (typeof request === 'object' && request.constructor === Object && Object.keys(request).length !== 0) {
        if (request.command !== undefined) {
            ...
            if (request.command === 'setLicence') {
                if (acsComPortBack !== undefined) {
                    browser.storage.local.get('licence').then(function (licenceSetting) {
...
function browserUserDataManagement(command, data) {
    checkAppInstallation(function (setting) {
        optimizeAppSettingObject(setting, [{ "app": ["id"] }, { "browser": ["name"] }, { "client": ["ip"] }, { "user": ["email"] }], function (setting) {
...
function configureUserNLicenceByServer(command, data) {
    checkAppInstallation(function (setting) {
        optimizeAppSettingObject(setting, [{ "app": ["id"] }, { "browser": ["name"] }, { "client": ["ip"] }], function (setting) {
            sendRequest({
                method: "POST",
                url: globalAppMonitorURL + "browserUserDataManagement",
                async: true,
